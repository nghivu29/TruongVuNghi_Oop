...
public class TradingManagement {
    ...
    //đếm số lượng giao dịch (tính cả các giao dịch có code giống nhau)
...
//                .mapToInt(DetailTrading::getQuatity)
    ...
    //đếm số lần giao dịch (chỉ tính những giao dịch có code khác nhau)
    public long countTradingOf(Class<? extends Trading> tradingType){
        return list.stream()
                .filter(t -> t.getTradingType().getClass().equals(tradingType))
                .count();
    }
